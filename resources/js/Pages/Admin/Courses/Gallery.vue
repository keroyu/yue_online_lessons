...
const toggleSelectAll = () => {
  selectedForDelete.value = isAllSelected.value
    ? new Set()
    : new Set(props.images.map(i => i.id))
}
...
const batchDelete = () => {
  ...
  router.delete('/admin/images/batch', {
    ...
    preserveScroll: true,
    onSuccess: () => { selectedForDelete.value = new Set() },
  })
}
</script>
    ...
    <div class="mt-8 bg-blue-50 rounded-lg p-4">
      ...
      <div class="mt-2 text-sm text-blue-700">
        <ol class="list-decimal list-inside space-y-1">
          <li>上傳圖片至相簿（支援多選或拖曳）</li>
          <li>點擊圖片勾選，再點「刪除已選」可批次刪除</li>
          <li>hover 圖片點複製按鈕取得 URL，於課程介紹插入：<code class="bg-blue-100 px-1 rounded">&lt;img src="圖片URL" width="100%" /&gt;</code></li>
        </ol>
      </div>
    </div>
  </div>
