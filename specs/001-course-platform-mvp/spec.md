...
4. **Given** 首頁「所有課程」上方有三顆分類按鈕（思維升級/財務覺醒/知識變現）, **When** 訪客點擊其中一顆, **Then** 下方課程列表前端即時只顯示該分類課程，該按鈕高亮（teal 底＋gold 底線）
5. **Given** 某分類按鈕為選中狀態, **When** 訪客再次點擊同一顆, **Then** 篩選清除、回到顯示全部課程
6. **Given** 訪客選了某分類但該分類目前無課程, **When** 篩選套用, **Then** 顯示「此分類目前沒有課程」而非空白
