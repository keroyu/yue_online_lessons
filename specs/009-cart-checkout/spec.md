...
**Updated**: 2026-05-07 - Phase 6-7 實作完成；新增 FR-032（金流設定狀態 hint）；課程金流選擇器改為 pill button；guest cart 登入合併完整閉環
**Updated**: 2026-05-07 - Phase 8-9 實作完成：NewebpayService（AES-256-CBC + PKCS7 手動 unpad + TradeSha 驗章）、NewebpayController（notify 回 `SUCCESS`、return 僅 redirect 不重複 fulfillOrder）、路由（notify 在 api.php、return 在 web.php withoutMiddleware CSRF）；.env.example 補齊 PAYUNI_* / NEWEBPAY_* / META_PIXEL_ID 佔位欄位
**Updated**: 2026-05-07 - 修正金流設定頁 HashKey/HashIV placeholder bug：未設定時顯示「尚未設定」，已設定時顯示前 5 碼 + 星號遮蔽；新增 FR-033；後台金流區塊標題統一格式為「中文名稱（英文）」
**Updated**: 2026-05-07 - 增量追加 US8：Portaly Webhook 加密金鑰移至後台設定（`site_settings`），與 PayUni/NewebPay 憑證設定同頁；新增 FR-034、FR-035、SC-010
**Updated**: 2026-05-07 - 修正 NewebpayService::decryptTradeInfo 解析方式：因 RespondType=JSON 時藍新加密的是 JSON 字串而非 query string，改用 json_decode 為主、parse_str fallback；修正前付款後 Webhook 永遠視為非 SUCCESS，導致會員付款後無課程（FR-039）
**Updated**: 2026-05-07 - 金流流程體質強化：購物車「付款失敗」flash 讀取路徑修正、ReturnURL 加 fallback fulfillOrder（NotifyURL 失敗時的安全網）、付款等待頁加 60 秒輪詢上限與超時 UI、登入頁 hint 接受 'purchase'；新增 FR-037、FR-038、FR-040
**Updated**: 2026-05-09 - 懸浮面板新增收合功能（FR-041）：右上角「›」收合按鈕，收合後右緣顯示垂直金色「購買」標籤可重新展開；新增 US1 AS-12
**Updated**: 2026-05-07 - 結帳頁新增「公司統編」optional 欄位（8 位數字，留空允許），記錄於 orders.tax_id；付款成功頁與後台交易詳情頁/CSV 同步顯示；新增 FR-036
**Status**: Draft
...
### Session 2026-05-07（增量更新）

- 決定：Portaly Webhook 加密金鑰（`PORTALY_WEBHOOK_KEY`）改為後台可設定，與 PayUni / 藍新憑證設定同頁（`/admin/settings/payment`）；`PortalyWebhookService` 改為優先讀取 `site_settings`，`.env` 僅作 fallback；行為與 FR-021/FR-022 完全一致。
...
12. **Given** 懸浮面板可見，**When** 用戶點擊右上角「›」收合按鈕，**Then** 面板滑出畫面右側，右緣出現垂直金色「購買」標籤；**When** 用戶點擊「購買」標籤，**Then** 面板滑回原位。
...
7. **Given** 結帳頁，**When** 用戶在「公司統編（如要報帳）」欄位輸入恰好 8 位數字，**Then** 欄位通過驗證、送出後該值寫入 `orders.tax_id`，並出現於付款成功頁、後台交易詳情頁與 CSV 匯出。
8. **Given** 結帳頁，**When** 用戶留空公司統編欄位，**Then** 視為未提供（`orders.tax_id = null`），不影響送出。
9. **Given** 結帳頁，**When** 用戶填寫的公司統編非 8 位數字，**Then** 欄位顯示紅色錯誤「統編需為 8 位數字」並 disable「前往付款」按鈕。
...
### User Story 8 — Portaly Webhook 金鑰後台設定 (Priority: P2)

管理員可在後台金流設定頁（與 PayUni / 藍新同頁）設定 Portaly Webhook 加密金鑰；設定後系統以資料庫值驗證來自 Portaly 的 webhook 請求，不再依賴 `.env`。

**Why this priority**: Portaly webhook 是 Portaly 課程訂單開通的唯一入口；金鑰需在部署後可由管理員維護，而不是每次修改都需要改 `.env` 重啟伺服器。

**Independent Test**: 在後台金流設定頁輸入 Portaly Webhook Key 並儲存，確認 `site_settings` 有對應 key；再用正確金鑰觸發一次模擬 Portaly webhook，確認驗證通過；修改金鑰後舊金鑰驗證應失敗。

**Acceptance Scenarios**:

1. **Given** 管理員進入 `/admin/settings/payment`，**When** 查看頁面，**Then** 頁面顯示「Portaly Webhook」區塊，含一個 Webhook Key 輸入欄位（`<input type="password">`）；已儲存時顯示前 5 碼加星號遮蔽預覽，尚未設定時顯示「尚未設定」placeholder。
2. **Given** 管理員在 Portaly Webhook Key 欄位輸入新值並提交，**When** 表單送出，**Then** 新金鑰儲存至 `site_settings`，頁面顯示成功提示。
3. **Given** Portaly Webhook Key 已設定於 `site_settings`，**When** Portaly 送出 webhook，**Then** 系統使用資料庫中的 Key 驗證簽章，而非 `.env` 的值；驗證通過後正常開通課程。
4. **Given** Portaly Webhook Key 尚未設定於 `site_settings` 但 `.env` 有值，**When** Portaly 送出 webhook，**Then** 系統 fallback 使用 `.env` 值驗證（不中斷現有運作）。
5. **Given** 管理員提交空白 Webhook Key 欄位，**When** 表單送出，**Then** 系統保留既有金鑰不覆寫（同 HashKey/HashIV 空白保留邏輯）。
...
- 公司統編為 optional 欄位；未填寫不阻擋結帳，僅在 `orders.tax_id` 留空。已填寫後在付款成功頁、後台交易詳情頁與 CSV 匯出皆可顯示／取得，方便企業會員報帳。
- 金流 NotifyURL（server-to-server）若因網路、伺服器繁忙或處理錯誤未成功 fulfill 訂單，ReturnURL（瀏覽器 redirect）會作為 fallback 觸發 `fulfillOrder()`；該方法已內建冪等檢查（`status === 'paid'` 即立即回傳），重複呼叫不會產生 duplicate Purchase。
- 付款等待頁輪詢 60 秒（30 次 × 2 秒）仍未確認時，UI 切換為 amber 警告，不再無限轉圈；提供重新確認、前往課程／回首頁兩種出口。
- 藍新金流的 `RespondType` 預設為 JSON，回傳的 TradeInfo 解密後是 JSON 字串而非 query string；解析時 MUST 用 `json_decode`，避免 `parse_str` 解析失敗造成 Status 永遠為 null。
...
- **FR-032**: 後台課程表單的「金流方式」選擇器 MUST 以 pill button 樣式呈現（非 `<select>` 下拉）；選中項目 MUST 顯示 indigo 高亮；選擇器旁 MUST 即時顯示該金流的設定狀態：當所選金流的 `merchant_id`、`hash_key`、`hash_iv` 任一在 `site_settings` 中為空時，MUST 在標籤旁顯示紅色警告文字並附帶「金流設定」頁連結（`/admin/settings/payment`）；設定完整時不顯示任何 hint。`gatewayConfigured` 狀態由 `CourseController` 讀取 `SiteSetting` 後注入 Inertia props，課程新增與編輯頁均適用。
- **FR-033**: 後台金流設定頁（`/admin/settings/payment`）的 HashKey / HashIV 輸入欄位 MUST 依儲存狀態顯示不同 placeholder：尚未儲存時顯示「尚未設定」；已儲存時顯示前 5 個字元加星號（例：`3oJT3***...*`），使管理員可確認已儲存但不洩漏完整憑證；`GET /admin/settings/payment` API 回應 MUST NOT 回傳明文，改以 `hash_key_preview` / `hash_iv_preview` 欄位（字串）回傳遮蔽後的預覽字串，空字串表示尚未設定。

- **FR-034**: 後台金流設定頁（`/admin/settings/payment`）MUST 新增「Portaly（Webhook）」區塊，提供一個 Portaly Webhook Key 輸入欄位（`<input type="password">`）；顯示規則與 HashKey/HashIV 一致（FR-033）：未儲存時 placeholder 顯示「尚未設定」，已儲存時顯示前 5 碼加星號；`GET /admin/settings/payment` API 回應 MUST NOT 回傳明文，改以 `portaly_webhook_key_preview` 欄位（遮蔽預覽字串，空字串表示尚未設定）回傳；提交時若欄位為空則保留既有值不覆寫。設定值以 key `portaly_webhook_key` 儲存至 `site_settings` 表。
- **FR-035**: `PortalyWebhookService::verifySignature()` MUST 改為優先從 `site_settings` 讀取金鑰（key: `portaly_webhook_key`），以 `config('services.portaly.webhook_key')` 作為 fallback；不得破壞現有 webhook 簽章驗證邏輯。
- **FR-036**: 結帳頁 MUST 提供 optional「公司統編（如要報帳）」欄位，留空允許；填寫時 MUST 為恰好 8 位數字（前後端雙重驗證），不符規格時 MUST 顯示錯誤提示並 disable 送出按鈕。資料 MUST 持久化於 `orders.tax_id`（varchar(8) nullable）；付款成功頁、後台交易詳情頁與 CSV 匯出 MUST 在有值時顯示此欄位。
- **FR-037**: 金流 ReturnURL（PayUni 與 NewebPay）在付款成功時 MUST 呼叫 `CheckoutService::fulfillOrder()` 作為 NotifyURL 失敗時的安全網（fallback fulfill）；`fulfillOrder` 須維持冪等（檢查 `order.status === 'paid'` 後立即回傳），確保重複呼叫不產生 duplicate Purchase 紀錄。
- **FR-038**: 付款成功頁（`/payment/success`）等待 overlay 在訂單仍 `pending` 時 MUST 每 2 秒輪詢 `GET /api/checkout/order-status`，但 MUST 設 30 次（60 秒）上限；超時後 MUST 停止輪詢並切換到 amber 警告 UI，提供「重新確認」與「前往我的課程／回首頁」按鈕，並附客服 Email。
- **FR-039**: `NewebpayService::decryptTradeInfo()` MUST 支援 RespondType=JSON（藍新預設）：解密後優先以 `json_decode()` 解析；若回傳非陣列則以 `parse_str()` 解析（兼容 RespondType=String）。修正前 `parse_str` 對 JSON 字串解析會失敗，導致 `Status` 永遠為 null，付款後不觸發 `fulfillOrder` → 會員無法取得課程。
- **FR-040**: 購物車頁 (`/cart`) MUST 從 `page.props.flash.payment_failed` 讀取 flash 訊息（透過 `HandleInertiaRequests` 共享），失敗時顯示紅色橫幅含失敗原因與客服聯絡 Email；登入頁 MUST 同時接受 `?hint=payuni` 與 `?hint=purchase` 兩種 query parameter，皆顯示「付款已完成！請用購買時填寫的 Email 登入查看課程」提示橫幅。
- **FR-041**: 課程銷售頁右側懸浮面板 MUST 提供收合按鈕（「›」圖示，絕對定位於面板右上角，`z-10` 確保在倒數計時元件之上）；點擊後面板以 `translate-x` 動畫滑出畫面右側，右緣同時出現垂直排列的金色「購買」標籤按鈕（`position: fixed; right: 0`）；點擊標籤後面板重新滑入。兩個狀態由單一 `<Transition mode="out-in">` + `v-if`/`v-else-if` + 不同 `key` 屬性控制，收合狀態由前端 `floatingCollapsed` ref 管理，不持久化。
...
- **SC-010**: Portaly Webhook Key 可在後台設定並即時生效（無需重啟伺服器）；`.env` 未設定值時，後台設定值為唯一憑證來源；`site_settings` 有值時，優先使用資料庫值，`.env` 值自動降為 fallback。
...
- PayUni 與藍新金流的 MerchantID、HashKey、HashIV 儲存於 `site_settings` 表，後台可設定；`.env` 中的對應變數僅作初始 seed 或 fallback，服務層優先讀取資料庫值。Portaly Webhook Key 同樣採用此模式，key 為 `portaly_webhook_key`，fallback 為 `config('services.portaly.webhook_key')`。
