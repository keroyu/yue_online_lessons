...
- [X] T025 [US3] Create `app/Http/Requests/CheckoutRequest.php` — validate `buyer.name` (required, max:100), `buyer.email` (required, email, max:255), `buyer.phone` (required, max:20), `agree_terms` (required, accepted), `course_ids` (required array min:1, each exists in courses and is purchasable)
...
- [X] T027 [US3] Add `buildOrderPaymentForm(Order $order, array $buyer): array` to `app/Services/PayuniService.php` — use `$order->merchant_order_no` as MerTradeNo, `$order->total_amount` as TradeAmt, `$order->buyer_email` as UsrMail, ProdDesc from first item name (+ " 等 N 門課程" if multi); preserve existing `buildPaymentForm(Course, ...)` signature untouched
...
- [X] T029 [US3] Register routes — `routes/api.php`: `POST /checkout/initiate` (public); `routes/web.php`: `GET /checkout` → CheckoutController@show (public)
