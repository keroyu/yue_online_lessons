...
6. **Given** 課程小節無影片但有 Markdown 內容, **When** 會員點擊該小節, **Then** 右側顯示該小節的 Markdown 內容渲染結果（電子書或文章形式）
...
管理員進入某課程的章節編輯頁面，可新增「章」和「節」。「章」用於劃分大單元，「節」為實際課程內容。新增小節時可輸入節名稱、選填 Vimeo/YouTube 影片連結，或輸入 Markdown 內容。完成編輯後可將課程從「草稿」發佈。
...
1. **Given** 管理員在課程介紹編輯頁, **When** 在編輯器中輸入 Markdown 內容, **Then** 儲存成功，課程販售頁顯示渲染後的內容
...
- **FR-007**: 小節如無影片，MUST 能顯示 Markdown 內容（渲染為 HTML 後呈現）
...
- **FR-016**: 小節 MUST 可設定 Vimeo/YouTube 連結或 Markdown 內容
