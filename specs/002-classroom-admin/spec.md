...
**Updated**: 2026-03-09 - 章節通知 Email 排除 drip 連鎖課程（FR-068）
...
### User Story 10 - 新增章節時 Email 通知擁有課程的會員 (Priority: P2)

管理員在已發布課程（預購中或熱賣中）中新增章節時，可選擇發送 Email 通知所有擁有該課程的會員。Email 內容說明哪門課程新增了哪個章節，並邀請會員回來繼續學習。此功能幫助提升舊學員的回訪率與課程完課率。

**Why this priority**: 對已購買學員的主動通知可提升學習黏著度，但屬加值功能而非核心流程，故定為 P2。

**Independent Test**: 管理員在已發布課程新增章節並勾選「發送通知」，驗證擁有該課程的會員收到 Email，Email 包含正確的課程名稱和章節名稱；草稿課程新增章節時不發信。

**Acceptance Scenarios**:

1. **Given** 管理員在已發布課程（預購中或熱賣中）的章節編輯頁新增章節, **When** 新增章節表單中勾選「發送 Email 通知學員」, **Then** 章節儲存成功後，系統發送通知 Email 給所有擁有該課程的非管理員會員
2. **Given** 管理員新增章節但未勾選「發送 Email 通知學員」, **When** 章節儲存成功, **Then** 系統不發送任何 Email
3. **Given** 通知 Email 已發送, **When** 會員開啟 Email, **Then** 信件內容包含課程名稱與新章節名稱，並附有前往上課頁面的連結
4. **Given** 管理員在草稿課程中新增章節, **When** 新增章節表單顯示, **Then** 「發送 Email 通知學員」選項不顯示（或為禁用狀態），不允許對草稿課程發通知
5. **Given** 某課程有 3 位付費學員和 1 位系統指派（管理員自己）, **When** 管理員勾選通知並儲存章節, **Then** 僅 3 位付費/贈送學員收到 Email，管理員本人不收
6. **Given** 會員已退款該課程, **When** 管理員發送章節新增通知, **Then** 該退款學員不收到 Email
7. **Given** 管理員在 drip 連鎖課程中新增章節, **When** 新增章節表單顯示, **Then** 「發送 Email 通知學員」選項不顯示，因為 drip 課程有自己的自動發信排程
...
- 章節新增 Email 通知：若某課程無任何符合資格的會員（全數退款或無人購買），勾選通知後靜默成功（不報錯、不發信）
- 章節新增後 Email 發送失敗時，不應影響章節本身的儲存成功；系統應記錄錯誤供管理員排查
- 管理員一次新增多個章節（或快速連續新增）時，每次儲存均需個別勾選，系統不合併批次發信
- 已退款（status = refunded）的購買紀錄，其對應會員不在通知名單中
- drip 連鎖課程新增章節時，「發送 Email 通知學員」選項完全不顯示，因 drip 課程依訂閱排程自動發信，手動觸發會造成訂閱者混亂
...
**章節新增 Email 通知（US10, 2026-03-09 新增）：**
- **FR-061**: 管理員在已發布課程（status = preorder 或 selling）新增章節時，新增章節表單 MUST 顯示「發送 Email 通知學員」勾選框，預設為未勾選
- **FR-062**: 若管理員勾選通知並成功儲存章節，系統 MUST 發送通知 Email 給所有符合條件的課程擁有者（購買紀錄 status ≠ refunded 且 type ≠ system\_assigned）
...
- **FR-068**: drip 連鎖課程（course_type = 'drip'）的新增章節表單 MUST NOT 顯示「發送 Email 通知學員」選項，後端亦 MUST NOT 發送通知，無論前端是否傳入 notify_members
...
- **SC-018**: 管理員勾選通知並儲存章節後，通知 Email 在 60 秒內送達所有符合資格的會員信箱
...
### Session 2026-03-09 (Update - 章節新增 Email 通知)

- Q: Email 通知是自動發送還是管理員選擇？ → A: 管理員 opt-in，新增章節表單有「發送 Email 通知學員」勾選框，預設未勾選
...
- Q: Email 發送是否為同步操作？ → A: 同步（Mail::send），章節儲存後在同一 request 內完成發信，與登入驗證碼發送方式一致；學員數量少，無需 Queue Job（若未來需要改為非同步，只需將 send() 改為 queue()）
