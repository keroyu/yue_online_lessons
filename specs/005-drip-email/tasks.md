...
- [x] T011 [P] Create drip lesson email Blade template with lesson title, full html_content (when present; stored as Markdown, rendered to HTML by SendDripEmailJob via CommonMarkConverter before passing to template), video notice (if has_video), fallback text for pure-video lessons without html_content ("本課程包含教學影片，請至網站觀看完整內容" with classroom link), classroom link (`/member/classroom/{course_id}`), and unsubscribe link (`/drip/unsubscribe/{token}`) in `resources/views/emails/drip-lesson.blade.php`
