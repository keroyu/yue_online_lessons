...
**Updated**: 2026-03-02 - 訂閱成功通知位置優化：通知移至主圖下方（頁面頂部可見），不再依賴 scroll；成功訊息加入訂閱者 Email；移除 scroll-to-bottom 代碼
**Updated**: 2026-03-02 - 驗證碼畫面加入寄件者提示：訂閱/登入驗證碼 Step 2 畫面加入小字提示「來信者為「經營者時間銀行」，找不到時請檢查垃圾郵件」
**Updated**: 2026-03-10 - 在 drip 信件末尾加入退訂連結（無 inline style），位於 tracking pixel 之前；管理員不需手動在 Lesson 內容中加入退訂連結
**Updated**: 2026-05-03 - Lesson 編輯器新增「插入教室連結」功能：drip 課程的 Lesson md_content 支援 `{{classroom_url}}` 佔位符，SendDripEmailJob 發信前替換為實際教室 URL（帶 lesson_id 參數）；LessonForm 提供快速插入按鈕並在偵測到影片 URL 時顯示琥珀色警示提示；CourseForm 連鎖 Email 設定相關文案微調（「目標商品」、「連鎖 Email 設定」）
...
1. **Given** 訪客在連鎖課程詳情頁，**When** 輸入新 Email 並完成驗證，**Then** 系統建立新會員帳號、建立訂閱記錄、**立即**發送歡迎信、解鎖第一個 Lesson；頁面頂部（主圖下方）顯示訂閱成功通知，包含訂閱者 Email
...
1. **Given** 已登入會員在免費連鎖課程詳情頁，**When** 點擊「訂閱」按鈕，**Then** 直接建立訂閱記錄、**立即**發送歡迎信、解鎖第一個 Lesson；頁面頂部（主圖下方）顯示訂閱成功通知，包含訂閱者 Email
...
5. **Given** 管理員在 drip 課程的 Lesson 編輯器填入有效影片連結，**When** 看到 Markdown 內容欄，**Then** 顯示琥珀色警示提示「記得加上教室連結」，並可點擊「+ 插入教室連結」在游標位置插入 `{{classroom_url}}`
6. **Given** 訂閱者的 drip email 內容含有 `{{classroom_url}}`，**When** 系統發送 Email，**Then** 佔位符被替換為實際教室 URL（帶 lesson_id 參數），格式：`{app_url}/member/classroom/{course_id}?lesson_id={lesson_id}`
...
4. **Given** 系統發出 drip email，**When** 收件者開信，**Then** 信件末尾顯示退訂連結，點擊後跳至退訂確認頁面
...
**驗證碼寄件者提示**
- **FR-079**: 所有需要 Email 驗證碼的步驟（訂閱 Step 2、登入/註冊 Step 2）MUST 在「驗證碼已發送至 {email}」文字正下方顯示小字提示，說明寄件者名稱為「經營者時間銀行」，並提示使用者檢查垃圾郵件

**Lesson 教室連結佔位符（2026-05-03 新增）**
- **FR-080**: `SendDripEmailJob` 在 render Markdown 前 MUST 執行 `str_replace('{{classroom_url}}', $classroomUrl, $rawMd)`；`$classroomUrl` 格式為 `config('app.url') . "/member/classroom/{$course->id}?lesson_id={$lesson->id}"`
- **FR-081**: Lesson 編輯器（drip 課程）SHOULD 在 Markdown 內容欄右上方提供「+ 插入教室連結」快速按鈕，點擊後在游標位置插入 `{{classroom_url}}`；偵測到有效影片 URL（`videoPlatform` 有值）時，SHOULD 在 Markdown 欄旁顯示琥珀色警示提示「此信有影片，記得用『插入教室連結』把觀看入口加入內容」；無影片 URL 時不顯示 hint
