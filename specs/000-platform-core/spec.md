...
| `ink-bamboo` | 墨竹 | 東方人文 — 書法、茶道、國學、古典樂 | `#FBF8F2` | `#585149` | `#557159` | `#EBD9B0` | `#DAC38F` | `#B9743C` | `#A8433A` |
| `deep-harbor` | 深港 | 商務財經 — 顧問、B2B、投資理財 | `#F2F5F7` | `#1B2A38` | `#2E5F8A` | `#C9A227` | `#B08F2A` | `#CA7827` | `#C93B3B` |
| `moss-field` | 苔原 | 健康永續 — 瑜珈、營養、園藝、戶外 | `#F8F8F1` | `#525948` | `#537645` | `#EBE3BB` | `#D8CD96` | `#B2763A` | `#AC4638` |
...
| `rose-quartz` | 玫瑰石英 | 美感生活 — 美妝、香氛、婚顧、花藝 | `#FDF8F6` | `#63505A` | `#9A5570` | `#F6DCC6` | `#E6C4A9` | `#BF7050` | `#B04B64` |
...
- **FR-120**: 所有配色 MUST 為淺底，且 `navy` MUST NOT 選得比門檻所需更暗。初版三組（墨竹／苔原／玫瑰石英）的 `navy` 亮度只有 0.018–0.025，而門檻允許到 0.157 —— 過暗的 `navy` 同時是導覽列底色與內文色，配上同樣深的主要按鈕，整頁讀起來就是兩塊黑板。`cream` 是 `body` 底色、`navy` 是後台側欄底色配白字，這兩個角色反向鎖死；深色主題必須同時把兩者對調，那是另一條故事而不是多一組色票
- **FR-121**: 收錄門檻（WCAG 2.1 相對亮度，全部由測試算出，非目測）。價格與分期價是 `text-xl`～`text-3xl` 的粗體，適用大型文字的 3:1；其餘一律 4.5:1（內文對底色原訂 AAA 7:1，實作後降為 AA 4.5，見 D48）。最後一條不是可讀性而是**辨識度**：導覽列與主要按鈕不得讀起來是同一塊色板：

  | 組合 | 用途 | 門檻 |
  |---|---|---|
  | navy / cream | 內文對頁面底色 | 4.5 |
  ...
  | navy / teal | 導覽列與主要按鈕的區辨 | 1.35 |

  逐組實測值由 `ColorSchemeTest` 現算，不在此重抄（抄下來只會過時）。
...
- **D48**: 內文對底色由 AAA 7:1 降為 AA 4.5:1，並把「`navy` 不得比門檻所需更暗」寫成規則（使用者回報三組配色「對比太高、導覽列與按鈕都是深色底、重複性高」後的修正）。根因有兩層：(1) 我在初版把 `navy` 選到亮度 0.018–0.025，而 7:1 的門檻其實允許到 0.083 —— 是我自己選得比規則更保守；(2) 7:1 這條門檻本身也選錯了，因為 `navy` **同時**是內文色與導覽列／側欄的底色，把內文推到 AAA 等於把一大片色塊一起推到近黑，再配上同樣必須承載白字的主要按鈕，整頁就是兩塊黑板。降到 AA 之後 `navy` 的可用亮度上限放寬 8 倍，三組重新配色後亮度落在 0.084–0.094（約為原來的 4 倍），對底色仍有 6.8–7.4。
  連帶發現一個相反方向的限制：`navy` 是 gold CTA 上的**文字**，所以 `navy` 每提亮一分，`gold` 就必須跟著提亮才維持 4.5 —— 亮度 0.15 時 `gold` 要亮到幾乎與 cream 無法區分，CTA 等於消失。0.07–0.09 是兩邊都站得住的區間，`gold` 因此一併改成淡赭／淡麥／淡蜜桃，這反而正是「輕盈」要的東西。新增的 `navy/teal ≥ 1.35` 門檻把使用者的抱怨直接編成把關，日後新增配色不會再犯。
...
- 2026-09-25: 重新設計 墨竹／苔原／玫瑰石英 三組配色（使用者回報「對比太高、導覽列與按鈕都是深色底、重複性高」）— 查證後根因有兩層，都在我這邊：初版的 `navy` 亮度只有 0.018–0.025，而當時 7:1 的門檻其實允許到 0.083（**我選得比自己訂的規則更保守**）；而 7:1 這條門檻本身也選錯了，因為 `navy` 同時是內文色與導覽列／側欄底色，把內文推到 AAA 等於把一大片色塊推到近黑，再配上同樣要承載白字的主要按鈕，整頁就是兩塊黑板。
  降到 AA 4.5 後重新配色：墨竹改宣紙＋淡墨（navy `#262521` → `#585149`）、苔原改亞麻＋石灰岩讓苔綠退成小面積重點（`#262E21` → `#525948`）、玫瑰石英的深梅底改灰褐玫瑰（`#3A2430` → `#63505A`）。三組 navy 亮度升到 0.084–0.094，約為原來的 4 倍。
  **提亮 navy 有個相反方向的連鎖**：`navy` 是 gold CTA 上的文字，navy 每亮一分 gold 就要跟著亮才維持 4.5 —— 量過之後亮度 0.15 時 gold 會亮到與 cream 幾乎無法區分，CTA 等於消失；0.07–0.09 是兩邊都站得住的區間。gold 因此一併改成淡赭／淡麥／淡蜜桃，反而正是「輕盈」要的。gold 的 hover 步階（1.25 倍）在淡色上視窗極窄，是解聯立算出來的不是試出來的（苔原的視窗一度是空的，改為提亮 gold 才打開）。
  新增門檻 `navy/teal ≥ 1.35`（導覽列與主要按鈕不得讀成同一塊色板），把使用者的抱怨編成把關。其餘五組未動且全數通過新門檻。**殘留限制**：只要導覽列是 `bg-brand-navy` 配白字、主要按鈕是 `bg-brand-teal` 配白字，兩者都必然落在亮度 0.18 以下 —— 要真正的淺色導覽列得改元件（淺底配 navy 字），那是配色換不掉的。全套 **1026 passed（4490 assertions）**、`npm run build` exit 0。
