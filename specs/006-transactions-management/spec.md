...
**Created**: 2026-03-10
**Updated**: 2026-03-11 - 交易列表「課程」欄位新增課程進度顯示（進度條 + X/Y 課）；交易列表每列新增「標記退款」快捷按鈕（置於「查看」按鈕左側）；修正「標記退款」按鈕 hover cursor 與「查看」連結一致；金額欄位強制顯示兩位小數（如 TWD 1200.00）
**Status**: Draft
...
1. **Given** 管理員已登入後台，**When** 進入交易紀錄頁面，**Then** 看到依購買時間倒序排列的交易列表，每筆顯示：購買者姓名/Email、課程名稱、實付金額（格式 `幣別 金額.00`，如 `TWD 1200.00`）、狀態（已付款/已退款）、類型（paid/system_assigned/gift）、購買時間。
