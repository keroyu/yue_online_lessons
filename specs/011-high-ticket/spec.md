...
- [x] `GET /booking/confirm/{token}` 為公開路由（無需登入）；確認成功後：`confirmed_at = now`、該 lead 的時段單位 `held_until` 清為 null（正式佔用），並顯示提醒頁（文案見 FR-181）
...
- [x] `confirmed` 與 `already` 兩種成功畫面在時段區塊下方顯示三點行前提醒（查收 Email／把時段排進行程／看完前置資料），措辭依狀態調整時態（FR-181）
...
送出後**一律等 8 秒**跑自動審核：滿 5 分進第二步，未達 5 分或勾選「目前沒有預算」則顯示婉拒文案。
...
- [ ] 收到回應後**不論通過與否一律顯示「自動審核中」畫面**（進度條 + 倒數秒數，長度 8 秒）；倒數結束後通過者進入第二步、未通過者顯示婉拒文案（FR-128）
...
- [x] 第五題答完後顯示顧問分派說明（灰色小字，不必勾，FR-166）與必勾的服務範圍告知（文案與版面見 FR-162），未勾時「送出審核」為 disabled；`?resume=` 回訪者與舊 lead **MUST NOT** 被這個勾選擋住（FR-164）
- [x] 伺服器端以 `accepted` 驗證 `screen_ack`，繞過前端直接打 `POST /course/{course}/screen` 而未帶勾選者回 422（FR-163）；該欄位不落庫、不加欄位
- [x] 回到第一步再往前 MUST NOT 要求重勾（狀態在精靈的 `form` 上，FR-165）
...
- [x] 快篩按鈕 MUST 互斥（同時只有一個生效），且點擊已生效的那顆 MUST 取消篩選回到不限；順序為 `[今日][近 7 日][明日]`（使用者指定）
...
- [x] 按鈕 MUST 有 hover 回饋與 `cursor-pointer`；手機寬度下 MUST 與其餘篩選元件一起換行不溢出
...
- **FR-143**: 客戶摘要 modal 的背景關閉 MUST 要求 **mousedown 與 click 都落在背景上**，且在草稿與已存內容不同時 MUST 先確認再關。前者是實際故障：在 textarea 裡拖曳選字、滑出面板才放開時，瀏覽器把 `click` 派送到 mousedown 與 mouseup 的共同祖先（即 modal 根節點），只看 `e.target === e.currentTarget` 的寫法會把一次選字判成「點了背景」，於是編輯中的摘要無聲消失。ESC 亦 MUST 忽略輸入法組字中的按鍵（`isComposing` / keyCode 229）—— 中文輸入時的 Esc 是取消候選字，不是關視窗
...
- **FR-123**: 資格審核共五題，題目、選項、儲存值與計分定版如下。滿分 10 分，**5 分（含）以上通過**（`BookingScreening::PASS_SCORE`）。標「否決」者無論總分多少一律不通過（使用者決策）。
  ...
  **Q2 `screen_budget` — 如果確認這項服務適合你，你目前可考慮投入的預算大約是多少？**
  ...
  | `none` | 目前沒有預算 | 0 **· 否決** |
  | `unsure` | 不確定，希望先了解方案內容與價格 | 1 **· 上限 6** |
...
- **FR-128**: 送出審核後 MUST **一律**顯示「自動審核中」畫面（進度條 + 倒數秒數），通過與否都一樣 —— 只有被擋的人要等，會讓兩條路徑的速度差自己說出答案。倒數長度為 **8 秒**（`REVIEW_SECONDS`，2026-08-17 由 60 秒縮短為 15 秒、2026-09-04 再縮短為 8 秒，皆為使用者決策：60 秒足以讓通過的人以為當掉了，15 秒仍偏長）。倒數 MUST 由前端執行，伺服器 MUST NOT sleep、MUST NOT 延遲回應：掛住一個 request 會佔用 PHP-FPM worker，而中途斷線會把已經完成的判定顯示成失敗。
...
- **FR-137**: `?resume=` 回站時，若該 lead 的既存答案**仍然通過**，`bookingDraft.screening_cleared` MUST 為 true，精靈據此**直接開在第二步**且不重跑審核倒數。
...
- **FR-144**: 預約名單 MUST 支援 `?met=` 面談時間快篩，值有三個：`today`、`7d` 與 `tomorrow`。
  `tomorrow` = `[明日 00:00, 後日 00:00)`，是唯一朝前看的一顆：明天的名單是**備課**（面談前要讀完對方的申請內容與過往摘要），與另外兩顆的**追銷**是兩種工作，所以它是獨立按鈕而不是把上界往後推 —— 把明天併進「近 7 日」只會讓一份追銷名單混進還沒發生的場次。
...
- **FR-150**: Leads 名單「序列信紀錄」欄的徽章 MUST 顯示 `{course.slug} ({emails_sent})`，MUST NOT 顯示課程全名。
...
- **FR-162**: 資格審核的第五題答完後 MUST 出現一則**必勾**的服務範圍告知，未勾選時「送出審核」按鈕 MUST 為 disabled。文案定版（一字不改）：
  > 請知悉：本諮詢不提供美食、旅遊、生活分享、網紅接案或純流量型帳號的定位建議。若您的主要目標是上述方向，本服務可能不適合您。

  呈現 MUST 醒目（有邊框與底色的區塊，不是灰色小字）—— 這條告知的作用是讓不適合的人**在填完之前就自己離開**，混在說明文字裡等於沒放。位置固定在五題之後、送出鈕之前：先讓人看完題目建立脈絡，再問他要不要繼續。
- **FR-166**: 顧問分派說明與上述勾選同時出現（`allAnswered` 之後），但 MUST 是勾選框**上方的灰色小字**，MUST NOT 併進勾選內容。文案定版（一字不改）：
  > 諮詢由本公司認證顧問提供，每位顧問皆由創辦人親自訓練、使用同一套診斷架構。創辦人親自主持的場次為限量名額，額滿後由認證顧問為您服務。

  兩者刻意分開（D125）：要人勾選同意的應該是「我知道這可能不適合我」這種風險告知，不是公司怎麼排班。把兩件事綁進同一個勾，等於要求對方在還沒決定申請前先同意一件對他不利的事，而勾選框越像賣家的免責條款集合，防禦心越高 —— 被稀釋掉的正是 FR-162 那條的自我篩選效果。
  文案本身的寫法也是同一個原則的產物：**MUST NOT** 使用「其他顧問」「專業顧問」這類措辭。「其他」定義了團隊相對於創辦人的位置（非主角），而「專業」是自稱的形容詞，兩者都會把剛壓下去的疑慮重新叫起來。改以「認證顧問」給身分、以「由創辦人親自訓練、使用同一套診斷架構」給同源承諾 —— 人怕的不是換人，是換到不一樣的答案。
  **前提**：「認證顧問」與「創辦人親自訓練」是**事實宣稱**，僅在確實如此時可用；這種話在面談現場被追問一次就穿幫，代價大於文案效果。
- **FR-163**: 勾選狀態以 `screen_ack` 隨審核請求送出，`BookingScreeningRequest` MUST 以 `accepted` 驗證擋住繞過前端的請求。
...
- **FR-172**: Q2 預算題答 `unsure`（不確定）時，總分 MUST 以 **6 分**為上限，其餘四題再滿分也一樣（使用者決策，2026-09-05 由 7 分下修為 6 分）。機制為選項上的 `cap` 旗標，與 `veto` 同層但語意較軟：`veto` 是無論如何都不通過，`cap` 是可以通過、但天花板被壓住。多個 `cap` 同時成立時取最低者。
  這條規則的目的不是扣分，是**分級**：8 分以上在後台顯示為「高購買意願」（FR-131），而一個還沒說出預算數字的人不該讓顧問帶著那個期待進面談。7 分與 6 分同屬「值得談」（5–7），因此下修**不改變徽章**，改變的是排序與比較 —— 名單依分數排時，講得出數字的 6 分和講不出數字的 6 分不該並列，而 7 分讓後者永遠排在多數前者之上。6 分把它壓到「值得談」的中段，離 5 分的及格線只剩一步。
  `unsure` 保留 1 分與不否決的處理不變（銷售頁隱藏價格，這仍是誠實回答）—— 變的只是它的天花板。上限 MUST 在 `BookingScreening::score()` 內套用，而不是在 `tier()` 或前端：落庫的 `screening_score` 只有一個，所有讀它的地方（分級、展開列的 `N/10`、日後任何報表）都必須看到同一個已經套過上限的數字。
  每次調整上限 MUST 附一次性回填 migration（`screen_budget='unsure'` 且 `screening_score>` 新上限者改為新上限），否則同樣的五個答案會因為送出時間落在部署前後而拿到不同分數，而那件事在列上完全看不出來。`down()` MUST NOT 還原 —— 原始總分隨時可由五題答案重算，寫回未設限的分數等於撤銷規則本身。
...
- **D125**: 顧問分派說明與服務範圍勾選**分開兩塊**（2026-09-02，使用者決策）。原本兩點併在同一個必勾項目裡，問題不在文字長度而在性質：勾選同意的東西應該是風險告知（「我知道這可能不適合我」），把「創辦人名額有限、額滿由他人服務」也塞進去，等於要對方在還沒決定申請前先同意一件對他不利的事。
  文案本身也重寫過一輪。原句「創辦人的限量名額釋出完畢時將由其他專業顧問服務您」的結構**預設了對方會失望** —— 一提醒，讀者才想到該失望。改法不是解釋得更完整（解釋越多越像道歉），是把預設值換過來：讓團隊服務是常態、創辦人場次是加值，句子裡就沒有降級這件事要解釋。定版見 FR-166。

- **D124**: 這條告知**不做成第六題計分題**（也不因為「主題是美食」就自動婉拒）。計分題問的是意願與能力，答案有高低之分；「你的帳號是什麼主題」沒有 —— 把它變成一個扣分選項，等於教會下一個人不要誠實回答。自我篩選在這裡比自動判定準確：真的做美食帳號的人看到這句話會自己走，而硬要留下來的人，五題計分本來就會接著處理。
  代價是它不進 `screening_score`、後台看不到「有多少人是因為這條告知離開的」。要量那件事得存下勾選與放棄的行為，那是另一個故事。
...
- [x] T336 摘要 modal 關閉守門（FR-143）：背景關閉改為 mousedown + click 都在背景才成立（並修好反向瑕疵 —— 背景層是獨立的 `fixed` 節點，先前點它其實關不掉）；ESC 略過 `isComposing` / keyCode 229；三個出口（ESC／背景／關閉鈕）統一走 `requestClose()`，草稿有變動時先 `confirm` in `resources/js/Components/Admin/Leads/ConsultationSummaryModal.vue`
- [ ] T337 使用者實測：在摘要 textarea 裡拖曳選字並滑出面板放開，modal MUST 不關；中文輸入法按 Esc 取消候選字，modal MUST 不關；改過內容後按關閉會先問
...
### US28 補充 — 明日快篩與序列信徽章（使用者追加）

- [x] T357 `metRange()` 加 `tomorrow` 分支（`[明日 00:00, 後日 00:00)` 台北），三個 preset 改由單一 `match` 同時回 from/until in `app/Services/HighTicketLeadService.php`
- [x] T358 `metPresets` 加「明日」（順序 `[今日][近 7 日][明日]`）in `resources/js/Components/Admin/Leads/BookingListTab.vue`
...
### 資格審核加服務範圍告知勾選（US24 追加 / FR-162–165）

- [x] T377 `screen_ack` 加入 `rules()`（`['accepted']`）與 `messages()`（`screen_ack.accepted` → 「請先勾選上方的服務範圍告知」）；**不動 `BookingScreening::rules()`**（FR-163/164）in `app/Http/Requests/BookingScreeningRequest.php`
- [x] T378 [P] `form` 加 `screen_ack: false`（不進 `book` / `waitlist` 的 payload，那兩支逐欄列舉，FR-165）in `resources/js/Components/Course/HighTicketBookingWizard.vue`
- [x] T379 五題全部答完後（`allAnswered`）於送出鈕前渲染必勾告知區塊：邊框＋淡色底的醒目樣式、`cursor-pointer`、文案照 FR-162 一字不改；`ready` 加上 `form.screen_ack` 條件；`errors.screen_ack` 有值時顯示 in `resources/js/Components/Course/BookingScreeningStep.vue`
- [x] T380 `submit()` 的 POST payload 加 `screen_ack`（`alreadyCleared` 短路路徑不受影響）in `resources/js/Components/Course/BookingScreeningStep.vue`
- [x] T381 `BookingScreeningTest` 補測試：未帶 `screen_ack` 的審核請求回 422 且不建立／不更新 lead、帶 `screen_ack=true` 且答案及格照常通過、送出申請（`book`）**不需要**帶 `screen_ack`（`?resume=` 與舊 lead 路徑不被擋）in `tests/Feature/HighTicket/BookingScreeningTest.php`
...
### 預算「不確定」的上限由 7 下修為 6（FR-172 修訂）

- [x] T406 `unsure` 選項的 `cap` 由 7 改 6（`score()` 的取最低者邏輯不動）in `app/Support/BookingScreening.php`
- [x] T407 一次性回填 migration：`screen_budget='unsure'` 且 `screening_score>6` 者改為 6（涵蓋 9/8 與上一版回填成 7 的那批），`down()` 不還原 in `database/migrations/2026_09_05_000001_lower_unsure_budget_screening_cap.php`
- [x] T408 `BookingScreeningTest` 上限測試期望值 7 → 6（實際只有「滿分組合」那條需要改：另兩條分別驗 5 分不被墊高與有講預算者不受限，都在新上限之下，原樣通過即為正確）in `tests/Feature/HighTicket/BookingScreeningTest.php`
- [x] T409 `php artisan test` 全綠（無前端變更）
- [ ] T410 使用者實測：正式站部署後查一筆 `screen_budget='unsure'` 的舊 lead，展開列顯示 6/10

### 預約確認頁加行前提醒（US11 追加 / FR-181）

- [x] T411 `confirmed` / `already` 兩個 view 的 `body` 收斂為一句話，另加共用的三點提醒陣列（查收 Email／排進行程／看完前置資料），依狀態切換時態；提醒區塊置於時段卡片下方，`expired` / `invalid` 不渲染 in `resources/js/Pages/Booking/Confirm.vue`
- [x] T412 `npm run build` exit 0；`php artisan test --filter=SlotHold` 確認確認流程未受影響
- [ ] T413 使用者實測：走一次確認連結看到三點提醒；同一個連結再點一次（`already`）也看得到

## 進度日誌

- 2026-09-07: 預約確認頁補上三點行前提醒（T411 / T412 完成，僅剩 T413 使用者實測，FR-181）— `confirmed` 與 `already` 各自的 `body` 收斂成一句，三件事改成編號清單放在時段卡片下方。兩態的提醒**分開寫而不是共用同一份**：`already` 的第一條時態不同（「先前已寄到你的信箱」），而共用一份再去改時態，等於把兩句不同的話塞進一個變數裡。`already` 原本的「不需要再做任何事」也一併改掉 —— 那句話把接下來該做的三件事整個否定掉了，而它偏偏是重複點連結回來的人唯一會讀到的一句。
  文案結構寫成 `{ label, text }` 物件而不是含 `**粗體**` 的字串：Vue 不會渲染 markdown，寫成字串就得 `v-html`，為了一個粗體開一道 XSS 的門不划算。`expired` / `invalid` 沒有 `reminders` 鍵，`reminders.length` 自然為 0，不必額外的狀態判斷。`npm run build` exit 0、`--filter=SlotHold` 14 passed、全站 `php artisan test` **871 passed（3630 assertions）**。

- 2026-09-07: [draft] 規劃預約確認頁的行前提醒（FR-181 / D129）— 確認成功頁現在只有一句「相關資料已寄出，建議在諮詢時間以前看完」，而這一頁是整段預約流程裡對方最後一次專心看畫面的時刻，三件該做的事（收信、排行程、讀資料）值得各佔一行。「排進行程」這句站得住是因為 `.ics` 由 `sendConfirmationMail()` 固定附加（FR-046），與模板無關。
  刻意不寫「影片」也不寫「已附於確認信中」（D129）：確認信本文存在 `email_templates`、後台隨時可改，程式碼這側看不到正式站當前那一版；把頁面文案綁在一段別人改得動的內容上，會在某次改模板之後變成叫人去信裡找一個不存在的東西，而且沒有任何測試會紅。依使用者決策改用通用的「前置資料」。提醒同時出現在 `confirmed` 與 `already`：重複點連結回來的人往往正是不確定自己該做什麼的那群。純前端、無 schema 變更、不動 email 模板。status: draft 待審核。

- 2026-09-05: 「不確定」上限由 7 下修為 6 完成（T406–T409，僅剩 T410 使用者實測）— 只改 `cap` 的值與一支回填 migration，`score()` 的「取最低者再 `min()`」邏輯與 `unsure` 的 1 分、不否決全部不動。
  回填條件寫成 `unsure 且 screening_score > 6`，一次收掉兩批：本來就是 8/9 分的，以及上一版回填寫成 7 的 —— 分數在資料庫裡沒有記錄它是哪一版規則算出來的，所以「大於新上限」既是最簡單的條件，也是唯一不需要知道來源的條件。
  測試只有一條需要改期望值：另兩條（5 分不被上限墊高、有講預算者仍是 10 分）都在新上限之下，原樣通過本身就是正確的驗證。全站 `php artisan test` **859 passed（3580 assertions）**，無前端變更。

- 2026-09-05: [draft] 規劃「不確定」上限由 7 下修為 6（FR-172 修訂，使用者決策）— 機制不動，只改 `cap` 的值。7 與 6 同屬「值得談」（5–7），所以徽章不變；變的是**排序與比較**：名單依分數排時，講得出數字的 6 分和講不出數字的 6 分不該並列，而 7 分讓後者永遠排在多數前者之上，6 分把它壓到及格線上方一步。
  需要注意的是回填要涵蓋**兩批**：原本就是 8/9 分的，以及上一版回填時被寫成 7 的 —— 條件寫成 `unsure 且 >6` 兩批一起收，不必區分。這也是把「每次調整上限都要附回填」寫進 FR-172 的原因：分數在列上看不出是哪一版的規則算出來的。無 schema 變更、無前端變更。status: draft 待審核。
...
- 2026-09-02: 資格審核加服務範圍告知勾選完成（T377–T382，僅剩 T383 使用者實測）— 第五題答完後於送出鈕上方出現金色底＋邊框的必勾區塊，未勾時「送出審核」為 disabled；`BookingScreeningRequest` 以 `accepted` 驗證 `screen_ack` 擋掉繞過前端的請求，不落庫、無 migration。勾選狀態放在精靈的 `form` 上而非步驟元件，來回切步驟不必重勾（`book` / `waitlist` 是逐欄列舉的 payload，不會順手帶上）。
  **實作中最容易踩的一步是「不要動 `BookingScreening::rules()`」**：那份規則被送出申請的 `HighTicketBookingRequest` 以 `rules(false)` 共用，把 `screen_ack` 放進去會讓 `?resume=` 回訪者與上線前的舊 lead 在第五步被一個他們沒看過的欄位擋死 —— 已用一條測試（送出申請不需帶 `screen_ack`）把這件事釘住。既有 12 條打 `/screen` 的測試共用 `screeningPayload()`，勾選一併加進那個 helper 的預設值。
  另補一條守門測試：`Schema::hasColumn('high_ticket_leads', 'screen_ack')` 必須為 false —— 它是閘門不是同意書。新增 4 個測試（BookingScreeningTest 24 passed），全套 836 passed（3450 assertions）、`npm run build` exit 0

- 2026-09-02: [draft] 規劃資格審核的服務範圍告知勾選（US24 追加）— 五題答完後、送出鈕之前加一則必勾告知，聲明本諮詢不提供美食／旅遊／生活分享／網紅接案／純流量帳號的定位建議。用必勾 checkbox 而非 010 那種「送出即同意」的被動揭露（D123）：領取表單刻意降摩擦，這裡的摩擦就是功能本身 —— 要的是那個「等一下，我講的是不是就是美食帳號」的停頓，而被動揭露會被整段跳過，跳過的正是唯一需要讀到它的那群人。不做成第六題計分題也不自動婉拒（D124）：計分題的答案有高低之分，「你的帳號是什麼主題」沒有，把它變成扣分選項只會教下一個人不要誠實回答。使用者決定**不落庫、不加欄位**（FR-163），代價是事後無法證明某位申請者確實看過 —— 這是期望管理的閘門，不是同意書。實作上唯一的陷阱是 `screen_ack` 不可放進 `BookingScreening::rules()`（FR-164）：那份規則被送出申請的 `HighTicketBookingRequest` 以 `rules(false)` 共用，放進去會讓 `?resume=` 回訪者與上線前的舊 lead 在最後一步被一個他們沒看過的欄位擋下來。
...
- 2026-08-23: Leads 預約名單的「諮詢時段」欄由最右移到最左（勾選框之後、姓名之前）—— US28 的今日／近 7 日快篩把這份名單變成一天的工作清單之後，用來辨識每一列的就是面談時間，它排在八個欄位的末端還被 `hidden lg:table-cell` 藏在捲動區右側，等於每次都要橫捲過去確認。欄位可見性與 breakpoint 未動。
