...
- 2026-09-07: 預約確認頁補上三點行前提醒（T411 / T412，FR-181）— `confirmed` 與 `already` 各自的 `body` 收斂成一句，三件事改成編號清單放在時段卡片下方。兩態的提醒**分開寫而不是共用同一份**：`already` 的第一條時態不同（「先前已寄到你的信箱」），而共用一份再去改時態，等於把兩句不同的話塞進一個變數裡。`already` 原本的「不需要再做任何事」也一併改掉 —— 那句話把接下來該做的三件事整個否定掉了，而它偏偏是重複點連結回來的人唯一會讀到的一句。
