...
管理員在後台建立或編輯課程時，可將課程類別設為「工作坊」。選擇工作坊後，出現一個開關：「隱藏價格（改為預約模式）」。開啟後，前台自動隱藏原價/優惠價、並將按鈕改為「立即預約」。其餘欄位（課程描述等）不受影響。

**Why this priority**: 這是整個功能的資料基礎，沒有工作坊類別，後續前台展示與預約流程均無法進行。

**Independent Test**: 在後台新增課程、選擇類別「工作坊」並開啟「隱藏價格」，儲存後資料庫正確寫入。

**Acceptance Scenarios**:

1. **Given** 管理員在課程編輯頁，**When** 選擇類別「工作坊」，**Then** 出現「隱藏價格（改為預約模式）」開關。
2. **Given** 管理員開啟隱藏價格，**When** 儲存課程，**Then** 資料庫記錄 `high_ticket_hide_price = true`。
3. **Given** 管理員關閉隱藏價格（工作坊顯示價格模式），**When** 儲存，**Then** 前台維持顯示原有價格與倒數，按鈕仍為「立即購買」。
4. **Given** 課程類別非工作坊，**When** 管理員儲存，**Then** 新增欄位不出現，原有行為不變。
...
**Independent Test**: 建立一個「工作坊（隱藏價格）」課程，瀏覽銷售頁，確認底部價格區塊已替換為說明文字，按鈕為「立即預約」。
...
- 工作坊無設定 Email 模板時，系統略過發送（不 crash），記錄警告日誌。
- 隱藏價格模式下，頂部資訊列的 PriceDisplay 也隱藏，避免洩漏價格。
- 管理員將已存在的非客製服務課程改為工作坊，前台即時反映；購買記錄不受影響。
...
- **FR-001**: 課程類別選項 MUST 新增「工作坊」，與現有講座/迷你課/完整課程並列。
- **FR-002**: 當課程設為工作坊時，管理員 MUST 能設定一個布林開關「隱藏價格（改為預約模式）」（預設關閉）。
...
- **Course（課程）**: 新增欄位：`type` 擴充工作坊選項、`high_ticket_hide_price`（布林）。
...
- 工作坊顯示價格模式下，行為與一般課程完全相同（包含現有的立即購買流程）。
